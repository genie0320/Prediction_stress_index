...
def main():
    wandb.init(project="stress-index-prediction", name="v16_6_mlp_svr_ensemble", config={
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(X, y)):
        ...
        # Step B. inner_val 분리 (Early Stopping 전용)
        X_in_tr, X_in_va, y_in_tr, y_in_va = train_test_split(
            X_tr_sc, y_tr_t, test_size=0.1, random_state=fold, shuffle=True
        )
    ...
    pd.DataFrame({'ID': test['ID'], 'stress_score': test_mlp_final}).to_csv('result/v16/submit_v16-6_mlp.csv', index=False)
    
    # 블렌딩
    test_blend = best_alpha * svr_test_pred + (1 - best_alpha) * mlp_test_pred
    test_blend_final = np.round(np.clip(test_blend, 0, 1), 2)
    pd.DataFrame({'ID': test['ID'], 'stress_score': test_blend_final}).to_csv('result/v16/submit_v16-6_blend_svr_mlp.csv', index=False)
