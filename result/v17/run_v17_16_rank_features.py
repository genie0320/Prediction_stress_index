# result/v17/run_v17_16_rank_features.py
# 목적: 18개 모든 피처에 대해 절대값 대신 Percentile Rank(순위) 변환(Uniform 분포)을 
# 적용한 뒤 SVR을 학습시켜 성능 향상을 도모한다. (Data Leakage 없음)
...
def run_svr(X, y, X_test, seeds):
    ...
    for seed in seeds:
        ...
        for tr_idx, va_idx in kf.split(X, y):
            ...
            # ── 핵심: 모든 피처를 Percentile Rank(0~1)로 변환 ──
            # train fold(약 2700개)를 기준으로 등수를 계산
            feat_scaler = QuantileTransformer(n_quantiles=1000, output_distribution='uniform', random_state=42)
            X_tr_s = feat_scaler.fit_transform(X_tr)
            X_va_s = feat_scaler.transform(X_va)
            X_te_s = feat_scaler.transform(X_test)
            
            # 타깃 변환 (Target은 v17-2와 동일하게 Normal 변환)
            qt = QuantileTransformer(n_quantiles=NQ_TARGET, output_distribution='normal', random_state=42)
...
def main():
    ...
    print("="*65)
    print(">>> [v17-16] 18개 전 피처 Percentile Rank 변환 + SVR")
    print("="*65)

    print("1. 피처 랭크 변환 및 SVR 3-Seed 훈련 중... (약 1~2분 소요)")
    oof_pred, test_pred = run_svr(X, y, X_test, SEEDS)
    
    mae = mean_absolute_error(y, oof_pred)
    print(f"\n[Result] Blended OOF MAE: {mae:.6f}")
    
    v172_baseline = 0.134652
    diff = mae - v172_baseline
    if diff > 0:
        print(f"    => 기존 스케일러(Robust) 대비: {diff:+.6f} (악화) ❌")
    else:
        print(f"    => 기존 스케일러(Robust) 대비: {diff:+.6f} (개선) ✅")
    ...
    submit_df.to_csv('result/v17/submit_v17-16_rank_features.csv', index=False)
    
    print("\n[Test Stats]")
    print(f"mean={submit_pred.mean():.5f}, std={submit_pred.std():.5f}, unique={len(np.unique(submit_pred))}")
    print("저장 완료: result/v17/submit_v17-16_rank_features.csv")
