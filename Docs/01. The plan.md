...
- 골밀도-연령 이외의 특성간 상관관계는 거의 존재하지 않음 → 복합지표 또는 특성추출을 적용해볼 필요성이 있어보이며, 단순회귀모델로는 설명이 힘들다는 것을 나타냄.
  - mean_working (0.18\*): stress_score와 가장 강한 양의 상관관계(0.18)를 가지며, 유의확률 p-value < 0.05로 통계적 유의성 있음.
  - age - bone_density (-0.88\*): 강한 음의 상관관계.
  - age - pulse_pressure (0.15\*): 나이가 들수록 맥압(수축기-이완기 차이)이 증가하는 건강 지표 경향이 통계적으로 유의미함
    ![alt text](img/image.png)

- p-value가 2.364e-16으로 0에 수렴하여 통계적으로는 **확실한 연관성(인과 경향)**이 존재. 단, 결정계수(r2)가 **0.0336 (3.3%)**으로 낮으므로 다른 다양한 복합 요인들(혈압, 골밀도, 당뇨 등)의 영향을 동시에 받고 있다는 뜻임.
